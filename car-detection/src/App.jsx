...
function App() {
  ...
  const [vehiclesDetected, setVehiclesDetected] = useState(null)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setVehiclesDetected(data.vehicles_detected)
      setNumVehicles(data.num_vehicles)
      setVehiclesList(data.vehicles || [])
    } catch (err) {
      console.error("Error detecting vehicles:", err)
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <div className="container mx-auto px-4 py-8 min-h-full">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-slate-400 text-lg max-w-2xl mx-auto">
              Upload an image and let our advanced detecter vehicles with position analysis
            </p>
          </div>
                            ...
                            <div>
                              <div className="flex items-center gap-3 mb-3">
                                ...
                                <div>
                                  <p className="text-emerald-200 font-semibold">
                                    {numVehicles} Vehicle{numVehicles !== 1 ? "s" : ""} Detected!
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="space-y-2">
                                {vehiclesList.map((vehicle, index) => (
                                  <div
                                    key={index}
                                    className="flex items-center justify-between bg-emerald-500/10 rounded-lg p-2"
                                  >
                                    <div className="flex items-center gap-2">
                                      <IoCarSport className="w-4 h-4 text-emerald-300" />
                                      <span className="text-emerald-200 text-sm capitalize">{vehicle.type}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      {vehicle.position === "left" ? (
                                        <FaArrowLeft className="w-3 h-3 text-cyan-300" />
                                      ) : (
                                        <FaArrowRight className="w-3 h-3 text-cyan-300" />
                                      )}
                                      <span className="text-cyan-200 text-sm capitalize">{vehicle.position}</span>
